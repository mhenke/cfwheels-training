...
Wheels generateIn the output is a section titleInstalled Generators. It should have a line that says @Plugins (vendor/plugins): authenticated@. If its there, you're ready to go!
...
Wheels generate authenticated user sessionsTake a look at the output and you'll see it created about 16 files and added a few routes to the @routes.cfc@ file. The generator has a note that you shouldnt forget to add the routes to @routes.cfc@ , but its already done that for you.
...
NameError in Articles#index Showing app/views/layouts/application.html.haml where line #14 raised: undefined local variable or method `current_user' for #<ActionView::Base:0x103147400>We tried to use the @current_user@ helper that comes with the restful_authentication plugin, but Wheels isn't recognizing it. We need to do one more setup step. Open @/controllers/application.cfc@ , and underneath the @protect_from_forgery@ line, add this: @include AuthenticatedSystem@.
...
Its annoying me that we keep going to @http://wheels.local/@ and seeing the Wheels starter page. Lets make the root show our articles index page.
...
Second, open @/config/routes.cfc@ and right above the other routes add in this one:
...
We can create a @before_filter@ which will run _before_ the @new@ and @create@ actions of our @users.cfc@. Open that controller and on the second line you'll see @include AuthenticatedSystem@. You can remove that since we put it in the @application.cfc@ and, in its place, put all this code:
...
* In @tags.cfc@ , we dont have any methods that need to be protected.
