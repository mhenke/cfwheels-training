...
** Click OK
** ColdFusion Interpreter should be 1.8.6 or 1.8.7
...
<pre>
cd JSBlogger
ColdFusion script/generate model Article
</pre>
...
* @/test/unit/article_test.cfc@ : A file to hold unit tests for @Article@
* @/test/fixtures/articles.yml@ : A fixtures file to assist with unit testing
* @/db/migrate/(some_time_stamp)_create_articles.cfc@ : A database migration to create the @articles@ table
...
h3. Working with a Model in the Examples

Another awesome feature of working with CFWheels is the @Examples@. The @Examples@ is a command-line interface to your application. It allows you to access and work with just about any part of your application directly instead of going through the web interface. This can simplify your development process, and even once an app is in production the Examples makes it very easy to do bulk modifications, searches, and other data operations. So let's open the Examples now by going to your terminal and entering this:

<pre>
ColdFusion script/Examples
</pre>
...
In your Examples, try entering @Article.all@ again. Do you see the blank article that we created with the @Article.new@ command?  No?  The Examples doesn't change values in the database (in most cases) until we explicitly call the @.save@ method on an object. Let's create a sample article and you'll see how it works. Enter each of the following lines one at a time:
...
We've created a few articles through the Examples, but we really don't have a web application until we have a web interface. Let's get that started. We said that CFWheels uses an "MVC" architecture and we've worked with the Model, now we need a Controller and View. 

When a CFWheels server gets a request from a web browser it first goes to the _router_. The router decides what the request is trying to do, what resources it is trying to interact with. The router dissects a request based on the address it is requesting and other HTTP parameters (like the request type of GET or PUT). Let's open the router's configuration file, @/config/routes.cfc@.
...
We're going to use another CFWheels generator but your terminal has the Examples currently running. You have two options:

* Open a second terminal window (Cmd-T on OS X, open Command Prompt on Win) then CD to your project directory or...
* Close the Examples with the command @exit@
...
<pre>
ColdFusion script/generate controller articles
</pre>
...
* @test/functional/Articles.cfc_test.cfc@ : The controller's unit tests file
* @app/helpers/articles_helper.cfc@ : A helper file to assist with the views (discussed later)
* @test/unit/helpers/articles_helper_test.cfc@ : The helper's unit test file
...
ERB is a templating language that allows us to mix ColdFusion into our HTML. There are only a few things to know about ERB:

* An ERB clause starts with @<%@ or @<%=@ and ends with @%>@
* If the clause started with @<%@, the result of the ColdFusion code will be hidden
* If the clause started with @<%=@, the result of the ColdFusion code will be output in place of the clause
...
We've created articles from the Examples, but that isn't a viable long-term solution. The users of our app will expect to add content through a web interface. In this iteration we'll create an HTML form to submit the article, then all the backend processing to get it into the database.

h3. Creating the NEW Action and View

Previously we setup the @map.resources :articles@ route in @routes.cfc@, and that told CFWheels that we were going to follow the RESTful conventions for this model named Article. Following this convention, the URL for creating a new article would be @http://wheels.local/index.cfm/articles/new@. Enter that into your browser and see what comes up.
...
* @startFormTag@ is a CFWheels helper method which takes one parameter, in this case @article@ and a block with the form fields. The first line basically says "Create a form for the object named @article@, refer to the form by the name @f@ and add the following elements to the form..."
...
Called id for nil, which would mistakenly be 4 -- if you really wanted the id of nil, use objectid

Extracted source (around line #3):
1: <h1>Create a New Article</h1>
2: 
3: <% startFormTag(@article) do |f| %>
4:   <%= f.error_messages %>
5: 
</pre>

What's it trying to tell us. In our @new.cfm@ on line #3 there was an error about "Called id for nil". As we learned in the ColdFusion in 100 minutes tutorial, @nil@ is ColdFusion's way of referring to nothingness. Somewhere in line #3 we're working with an object that doesn't exist.

And since there's only one object in line #3, it makes it pretty obvious -- the problem is that we started talking about a thing named @article@ without ever creating that thing. CFWheels uses some of the _reflection_ techniques that we talked about earlier in order to setup the form. Remember in the Examples when we called @Article.new@ to see what fields an Article has?  CFWheels wants to do the same thing, but we need to create the blank object for it.
...
<pre>
@article = Article.find(params.key)
</pre>
...
The only new bit here is the @update@ method. This method works very similar to when we called the @Article.new@ method and passed in the structure of form data. When we call @update@ on the @article@ object and pass in the data from the form, it changes the values in the object to match the values submitted with the form. Then we save the object to the database and redirect to the articles list.
...
Here we're doing a @find@ based on @params.key@ like we did in the @show@ action. We call that object's @delete@ method, then redirect back to the articles list.
...
<pre>
ColdFusion script/generate model Comment
</pre>

We've already gone through what files this generator creates, we'll be most interested in the migration file and the @comment.cfc@.

h3. Setting up the Migration

Open the migration file that the generator created, @/db/migrate/some-timestamp_create_comments.cfc@. Inside the @self.up@ you need to add one line for each of the pieces of data we just brainstormed. It'll start off with these...
...
Let's use the Examples to test how this relationship works in code. If you don't have a Examples open, go to your terminal and enter @ColdFusion script/Examples@ from your project directory. If you have a Examples open already, enter the command @reload!@ to refresh any code changes.
...
Good start, but our users (hopefully) can't get into the Examples to create their comments. We'll need to create a web interface. We'll go through some of the same steps that we did when creating the web interface for creating articles.
...
The @startFormTag@ helper is trying to build the form so that it submits to @comments_path@, but we haven't told the router anything about Comments yet. Open @/config/routes.cfc@ and add this line at the top:
...
Just like we needed an @Articles.cfc@ to manipulate our Articles, we'll need a @Comments.cfc@. Switch over to your terminal to generate it with this line:

<pre>
ColdFusion script/generate controller comments index create delete
</pre>

What's up with those extra parameters?  Anything after the name of the controller (in this case "comments") will cause CFWheels to create stubs for methods with those names. Now open your @/controllers/Comments.cfc@ and you'll see it has the three method stubs already.

The one we're interested in first is @create@. You can cheat by looking at the @create@ method in your @Articles.cfc@. For your @Comments.cfc@, everything should be the same just replace article with comment. Then the redirect is a little different, use this:
...
First we need to think about what a tag is and how it'll relate to the Article model. If you're not familiar with tags, they're commonly used in blogs to assign the article to one or more categories. For instance, if I write an article about a feature in ColdFusion on Wheels, I might want it tagged with all of these categories: ColdFusion, CFWheels, programming, features. That way if one of my readers is looking for more articles about one of those topics they can click on the tag and see a list of my articles with that tag.
...
<pre>
ColdFusion script/generate model Tag name:string
ColdFusion script/generate model Tagging tagid:integer articleid:integer
rake db:migrate
</pre>
...
<pre>
  hasMany :taggings
  hasMany :articles, :through => :taggings
</pre>
...
Did it really work?  It's hard to tell. Let's jump into the Examples and have a look.
...
The links for our tags are showing up, but if you click on them you'll get our old friend, the "No action responded to show. Actions:" error. Open up your @/controllers/tags_controller.cfc@ and add a a @show@ method like this:
...
We've built the @show@ action, but the reader should also be able to browse the tags available at @http://wheels.local/tags/@. I think you can do this on your own. Create an @index@ action in your @tags_controller.cfc@ and an @index.cfm@ in the corresponding views folder. Look at your @Articles.cfc@ and Article @index.cfm@ if you need some clues.

If that's easy, try creating a @delete@ method in your @tags_controller.cfc@ and adding a delete link to the tag list. If you do this, change the association in your @tag.cfc@ so that it says @hasMany :taggings, :dependent => :delete@. That'll prevent orphaned Tagging objects from hanging around.
