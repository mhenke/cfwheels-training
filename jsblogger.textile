...
* Add this code at the very bottom:<br/>@<%= link_to "Create a New Article", new %>@<br/>which uses the CFWheels @link_to@ helper, tells it we want a link with the text "Create a New Article" that points to the address @new@ (which the router handles for us)
