...
This line tells CFWheels to do a lot of work. It declares that we have a resource named @articles@ and the router should expect requests to follow the *RESTful* model of web interaction (REpresentational State Transfer). The details don't matter to you right now, but just know that when you make a request like @http://wheels.local/articles/@ the router will know you're looking for a listing of the articles or @http://wheels.local/articles/new@ means you're trying to create a new article.
...
The first feature we want to add is an "index" page. This is what the app will send back when a user requests @http://wheels.local/articles/@ -- following the RESTful conventions, this should be a list of the articles. So when the router sees this request come in, it tries to call the @index@ action inside @articles_controller@.

Let's first try it out by entering @http://wheels.local/articles/@ into your web browser. You should get an error message that looks like this:
