...
During this training, I noticed a "bug" with the @renderPage@ and @#includePartial(article.comments)#@ combination. It seems @includePartial@ used this way doesn't allow another controller to be called so, I copied @/views/articles/_comment.cfm@ to @/views/comments@. This allows error messages to appear along with the form being populated again. Another fix could be to do a @redirectTo@.
