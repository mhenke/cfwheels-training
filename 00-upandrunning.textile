...
Now you'll see that the @findAll()@ command gave you back an query object holding the one article we created and saved. Go ahead and *create 3 more sample articles*.
...
Inside this file you'll see @<cfset addRoute(name="home", pattern="", controller="wheels", action="wheels")>@. This is the default route and is the Congratulations page, you saw when we first loaded our application. Let's replace it with:
