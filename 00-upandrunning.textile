...
Part of the reason ColdFusion on Wheels became popular quickly is that it takes a lot of the hard work off your hands, and that's especially true in starting up a project. CFWheels practices the idea of "sensible defaults" and tries to by creating a working application ready for your customization.

h3. Setting the Stage

First we need to make sure everything is setup and installed. See the "00-c-preparation.textile" page for instructions on setting up and verifying your ColdFusion, CFWheels, MySql, and add-ons.

With that done we need to load our tables into MySql

h4. Open MySql Workbench and...

* Load and run the sql jsblogger.sql
...
** Location: C:\Program Files\Apache Software Foundation\Tomcat 6.0\webapps\railo
** Project type: CFEclipse application
** Click OK

h3. Creating the Article Model

Our blog will be centered around "articles," so we'll need a table in the database to store all the articles and a model to allow our CFWheels app to work with that data. We'll the required files for our application. Switch to Eclipse and enter the following:
