...
We're going to use the Scaffold plugin again through the browser. Let's load @http://wheels.local/@. You'll notice our updated default route is mapping the code to the @index@ action of the @Articles@ controller but we are getting ann error since the view doesn't exist.
...
Did you catch it wasn't an error about the missing @Articles@ Controller?It is an error about a missing view. Wheels is smart enough for prototyping a skeleton application without any controller logic (actually the controller doesn't even to need to exist like this example).

In your browser, enter this url to get to the Scaffold plugin again. @http://wheels.local/index.cfm?controller=wheels&action=wheels&view=plugins&name=scaffold@:
