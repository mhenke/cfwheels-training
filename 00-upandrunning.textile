...
The first line was just to demonstrate that we can do anything we previously did during @CFML in 100 minutes@ and now inside our @controller@. The second like referenced the @Article@ model and called the @findAll@ method which returns a query of all articles in the database -- so far an empty result. The third line created a new article object. You can see that this new object had attributes @id@, @title@, @body@, @created_at@, and @updated_at@.
...
We're going to use another CFWheels generator but your browser has the console currently running. You have two options:

* Open a second browser window (Cmd-T on OS X, open Command Prompt on Win) then CD to your project directory or...
* Close the console with the command @exit@

I like to have several browser windows available to me when developing, so I'd always choose the first option.

In your browser, enter this command:
