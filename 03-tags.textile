...
When we start thinking about the database modeling, there are a few ways to achieve this setup. One way is to create a "join table" that just tracks which tags are connected to which articles. Traditionally this table would be named @articlestags@ and Rails would express the relationships by saying that the Article model @hasAndBelongsToMany@ Tags, while the Tag model @hasAndBelongsToMany@ Articles but Wheels doesn't have this relationship yet.
...
We will also need to call this new method within our @create@ and @update@ actions by adding @<cfset newTag()/>@. Your form should now show up and there's a text box at the bottom named "New Tag". Enter content for a new article and in the tag enter @ColdFusion@. Click SAVE and you'll get an successful massage.

Check to make sure your new tag is present and checked in the new article. Let's test adding an already existing tag in the new tag field along with leaving it blank with your sample article.
...
So looking at our pseudo-code, the next step is to go through @each@ of those @tag_names@ and find or create a tag with that name. Wheels has a built in method to do just that, like this:
...
The @build@ method is a special creation method. It doesn't need an explicit save, Wheels will wait to save the Tagging until the Article itself it saved. So, putting these pieces together, your @tag_list=@ method should look like this:
...
And you'll see the Tag is associated with just one Article.
