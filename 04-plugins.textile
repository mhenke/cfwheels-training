...
First well add the ability to upload the file when editing the article, then well add the image display to the article show template. Open your @/app/views/articles/_form.cfm@ view template. We need to make two changes
...
Ok, its in there, but we need it to actually show up in the article. Open the @/app/views/articles/show.cfm@ view template. In between the line that displays the title and the one that displays the body, lets add this line:
...
So open that @/app/views/articles/_form.cfm@ and look at the paragraph where we added the image upload field. Well add in some new logic that works like this:
...
When youshowan article that doesnt have an image attached it'll have an ugly broken link. Go into your @/app/views/articles/show.cfm@ and add a condition like we did in the form so the image is only displayed if it actually exists.
